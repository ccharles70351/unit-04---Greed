...
namespace Unit04.Game.Services
{
    ...
    public class VideoService
    {
        ...
        public void DrawActors(List<Actor> actors)
        {
            ...
            {
                actor.SetPosition(new Point(actor.GetPosition().GetX() + actor.GetVelocity().GetX(), actor.GetPosition().GetY() + actor.GetVelocity().GetY()));
                if (actors[1].GetPosition().GetX() == actor.GetPosition().GetX() && actors[1].GetPosition().GetY() == actor.GetPosition().GetY())
                {
                    Program.cast.RemoveActor("minerals", actor);
                    if (!actors[1].Equals(actor))
                    {
                        actors[1].SetScore(actor._score);
                    }
                }
                if (actor.GetPosition().GetY() > 1200)
                {
                    actor.SetPosition(new Point(actor.GetPosition().GetX(), 0));
                }
                actors[0].SetText((actors[1]._score + ""));
                DrawActor(actor);
            }
        }
